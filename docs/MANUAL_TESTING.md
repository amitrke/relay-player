...
| ❌ | **D-pad navigation does not work.** Confirmed on a real Chromecast with Google TV and on a Google TV emulator: a D-pad press scrolls the page instead of moving focus, and library content cannot be selected at all. Three causes, all in shipped code rather than missing Phase 4 work — see the measured note in architecture.md §11 |
