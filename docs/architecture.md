...
- The banner assets already exist (`assets/icon/tv_banner_*.png`, generated by `tool/tv_banner.py`) and are intentionally unreferenced. The manifest work is a `LEANBACK_LAUNCHER` category, `android:banner`, and `leanback`/`touchscreen` declared `required="false"` — held back until this phase because the filter alone would make the app launchable on a TV with a UI that has no D-pad focus traversal. See `assets/icon/README.md`.
